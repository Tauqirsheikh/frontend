...
export default function DocumentTypes() {
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            await bulkDeleteDocumentTypes(selectedIds);
            toast.success(`Successfully deleted ${selectedIds.length} document type(s)`);
            setSelectedIds([]);
                ...
                <div className="fixed inset-0 bg-slate-900/40 dark:bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white dark:bg-slate-900 rounded-2xl w-[420px] max-w-full shadow-2xl animate-in fade-in zoom-in-95 duration-200 overflow-hidden border border-slate-100 dark:border-slate-800 p-6">
                        <div className="flex items-center gap-3 mb-4 text-red-600 dark:text-red-400">
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-slate-800 dark:text-white">Delete Selected Records</h3>
                                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">This action cannot be undone.</p>
                            </div>
                        </div>
                        <p className="text-sm text-slate-600 dark:text-slate-300 mb-6">
                            Are you sure you want to delete <strong className="text-slate-800 dark:text-white">{selectedIds.length}</strong> selected record(s)?
                        </p>
